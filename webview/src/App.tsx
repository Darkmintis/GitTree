...
import { GitData, RepoGitData } from '@shared/types';
import './App.css';
  ...
  branches: [
    {
      ...mockData.branches[0],
      name: 'main',
      commits: mockData.branches[0].commits.slice(0, 5),
    },
  ],
};

const mockRepos: RepoGitData[] = [
  { id: 'demo-alpha', name: 'alpha-app', path: '/demo/alpha-app', data: mockData },
  { id: 'demo-beta', name: 'beta-lib', path: '/demo/beta-lib', data: mockDataB },
];

declare function acquireVsCodeApi(): { postMessage: (msg: unknown) => void };

export default function App() {
  const { setRepos, setError, setLoading } = useTreeStore();
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message.type === 'gitData') {
        if (Array.isArray(message.repos) && message.repos.length) {
          setRepos(message.repos as RepoGitData[]);
        } else if (message.data) {
          setRepos([{ id: 'workspace', name: 'repository', path: '', data: message.data as GitData }]);
        }
